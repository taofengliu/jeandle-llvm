...
// Cap on array length for virtualization candidates. Mirrors the
// "MaximumEscapeAnalysisArrayLength" guidance. Promoted to a
// hidden cl::opt so tests can override it without rebuilding; the default
// (128) matches Graal's MaximumEscapeAnalysisArrayLength
// (GraalOptions.java:104). A JVM-tunable surface
// (-XX:JeandleMaxEscapeAnalysisArrayLength=N) can map onto this later
// without churn to the data structures.
static llvm::cl::opt<unsigned> MaximumEscapeAnalysisArrayLength(
    ...
    llvm::cl::desc("PEA: cap on array length eligible for virtualization. "
                   "Larger arrays bypass PEA. Matches Graal's "
                   "MaximumEscapeAnalysisArrayLength (default 128)."));
...
// preserving virtualisation for objects allocated outside the nest. Mirrors
// Graal's GraalOptions.EscapeAnalysisLoopCutoff (default 20).
//
...
// within a single processLoop fixpoint. Both bounds match Graal's defaults
// (depth cutoff at EffectsClosure.java:413, inner-iter cap at line 458).
static llvm::cl::opt<unsigned> JeandlePEALoopCutoff(
    ...
    llvm::cl::desc("PEA: loop nesting depth threshold. When a nest's maximum "
                   ...
                   "existing virtuals). Matches Graal's "
...
// every Java method". Used by Graal-parity tooling to focus PEA on a
// specific compilation, and by lit tests that want to exercise gating.
...
//     proxy for Graal's MonitorIdNode.getLockDepth() and is
//     still consulted by blockExitInfoEquivalent comparisons of LiveLockEnters
//     across loop-fixpoint iterations (where it is intentionally IGNORED —
...
// stack-identity) compare BytecodeDepth, recovering Graal parity when the
// metadata is supplied and otherwise degrading to the existing Order proxy.
struct LockEnter {
  llvm::CallBase *Call;
  uint32_t Order;
...
struct BlockExitInfo : BlockExitData {
  ...
  // successor). Graal references:
  //   - EffectsClosure.processBlock exception handling
  //                                              EffectsClosure.java:288-300
  //   - EffectsClosure.afterSplit                EffectsClosure.java:313-320
  //   - PartialEscapeBlockState.exceptionEdgesToKill
  //                                              EffectsBlockState.java:33-46
  //
  ...
  // pred as contributing nothing on that path (equivalent to Graal's
  // PartialEscapeBlockState.exceptionEdgesToKill + EffectsClosure.afterSplit
  // markAsDead).
...
public:
  ...
  // Three states mirroring Graal's EffectsClosureMode (EffectsClosure.
  // java:331-345):
...
private:
  ...
  // Each entry also carries a per-run-monotonic Order tag (see LockEnter)
  // serving as Graal's MonitorIdNode.getLockDepth() stand-in for the narrow
  // cascade rule. back().Order == this VO's max lock depth, front().Order ==
  // this VO's min lock depth.
  DenseMap<jeandle::ObjectID, SmallVector<LockEnter, 4>> LiveLockEnters;
  ...
  // processLoop entry to keep the bound per-nest. Mirrors Graal's
  // EffectsClosure.java:553-564 (tooManyIterationsSeen).
  ...
  // processLoop entry. Stands in for Graal's
  // EffecsClosureOverflowException unwinding mechanism (no real exceptions
  // here — Jeandle is C++ -fno-exceptions; a polled flag is equivalent
  // since processLoop is the only consumer).
  ...
  // fresh ID for the same body-local alloc. Mirrors Graal's invariant that
  // virtualizing the same CommitAllocationNode always yields the same
  // VirtualObjectNode. This cache is monotonically grown (never cleared
  // across iterations or runs of the analyzer).
  ...
  // BlockExits entry on at least one iteration. The fixpoint invariant —
  // mirroring Graal's EffectsClosure knownAliveLoopEnds — is that once a
  // loop block has produced exit state, subsequent iterations must continue
  ...
  // Case A per-pred materialize path. Mirrors Graal
  // PartialEscapeClosure.MergeProcessor.processPhi (lines 1377-1515) and
  // mergeObjectStates (lines 1102-1332), restricted to non-self-reference,
  ...
  enum class MatReason : uint8_t {
    Unknown,
    Unhandled,   // unhandled escape-point instruction (Graal's "virtualize
                 // returned false" generic path).
  ...
  // multiple times. Mirrors Graal
  // PartialEscapeClosure.processStateBeforeLoopOnOverflow
  // (PartialEscapeClosure.java:229-237).
  ...
  // re-virtualisation. Matches Graal's PartialEscapeBlockState COW model:
  // each iteration's per-VO ineligibility flips are local to that iteration
  // and only persist if the converged exit state requires them.
  struct LoopSnapshot {
    jeandle::PEABlockState CurrentState;
    jeandle::AliasMap Aliases;
    DenseMap<jeandle::ObjectID, DenseMap<int64_t, jeandle::FieldValue>>
  ...
  // fold). Returns false if the dst pointer is NOT a virtual: caller falls
  // through to the generic-escape path which still materializes a virtual
  // SOURCE if any.
  bool tier2ArrayCopy(MemTransferInst *MI);
  // llvm.memset on a virtual array.
  bool tier2MemSet(MemSetInst *MSI);
...
void Analyzer::processBlock(BasicBlock *BB) {
  ...
  if (TermII) {
    ...
    // contribution to the unwind successor (mirrors Graal's
    // exceptionEdgesToKill + afterSplit markAsDead semantics).
...
void Analyzer::mergeStates(BasicBlock *BB) {
  ...
  // |offsets|) per-VO merge cost. Mirrors Graal's
  // PartialEscapeBlockState.identicalObjectStates +
  // adoptAddObjectStates fast path (PartialEscapeClosure.java:935-936).
  ...
  // intersection may remain unified at BB's entry — Graal's
  // MergeProcessor.intersectVirtualObjects (PartialEscapeClosure.java:
  // 1055-1087). Any ID present on some preds but not all is dropped
  ...
  // that the per-block EliminateAllocation effect kills entirely —
  // matching Graal's behaviour where the loop header's MergeProcessor
  // intersection is exactly the set that persists into the loop body.
  ...
  //
  // To converge to Graal's MergeProcessor.merge fixpoint
  // (PartialEscapeClosure.java:932-1052: `do { ... } while (materialized)`),
  ...
  do {
    ...
    if (!AllVirtual) {
      ...
      // (Graal's MergeProcessor.merge:982-1003) is deferred to a follow-on
      // task; the current code keeps the conservative bail.
      jeandle::VirtualObject &VObj = *Result.VirtualObjects[ID];
      if (VObj.IsSynthetic) {
        Eligible[ID] = false;
      ...
      if (!VObj.AllocationCall ||
        ...
        // "Mixed-merge non-dominating alloc": the proper fix
    ...
    if (!LocksMatch) {
      // Lock-count cascade at merge. Graal
      // (PartialEscapeClosure.MergeProcessor.merge:979-1003) handles a
      // virtualCount==N + locksEqual==false merge by falling into the
      // "materialize each virtual pred + build a materializedValuePhi"
      // branch. Each ensureMaterialized at the pred carries the PRED's OWN
      // lock list into materializeWithCommit
      // (PartialEscapeBlockState.materializeWithCommit), and the resulting
      // CommitAllocationNode emits exactly the pred's own
      // monitorenter set — no synthesized enters are added on the
      // lower-count side, so user semantics are preserved (the lower-count
      // path keeps having fewer real enters than the higher-count one).
      //
      // Jeandle's mechanism (materializeAtPredFromExitInfo's un-elide
      // path) is the exact analogue: for each pred whose VO is virtual,
      // call materializeAtPredFromExitInfo so the pred's OWN elided enters
    ...
    //
    // Switch the per-element comparison to (Call, BytecodeDepth)
    // — Graal's locksEqual semantic when the JDK frontend's depth metadata
    // is present (otherwise the BytecodeDepth field carries the Order proxy
    // value at push time, recovering exactly the Call-only check
    // for lit tests / stale frontends). On mismatch, do NOT mark the VO
    // ineligible: route to the same per-pred materialize path that the
    // LockCounts-differ branch above uses (Graal-parity recovery —
    // PartialEscapeClosure.MergeProcessor.merge:979-1003 cascades to
    // per-pred ensureMaterialized when locksEqual is false).
    if (RefLC != 0) {
      ...
      for (const auto *P : Preds) {
        ...
        for (unsigned i = 0; i < S.size(); ++i) {
          ...
          // — but Graal's MergeProcessor would have cascaded per-pred and
          // produced lock-correct IR.
  ...
  for (PHINode &Phi : BB->phis()) {
    ...
    if (AllSame && First) {
      ...
      if (OS && OS->isVirtual()) {
        ...
        // PEA. Mirrors Graal's deleteNode(phi) at
        // PartialEscapeClosure.java:1410-1413.
    ...
    // all equal. Attempt to synthesize a single merged VirtualObject (Graal
    // PartialEscapeClosure.MergeProcessor.processPhi:1408-1488). On success
    // the PHI is aliased to the new VO and downstream uses fold through it.
    // On failure (compatibility, identity, or per-entry type checks) we fall
    // through to Case A.
...
bool Analyzer::synthesizeCaseC(
  ...
  // Compatibility check — mirrors Graal processPhi:1418-1430.
  ...
  // Lock compatibility — Graal processPhi:1426 (locksEqual).
  ...
  // users — the merged virtual is treated as a structural value carrier,
  // not an identity carrier (Graal ObjectEqualsNode.java:157-195 plus
  // BoxNode.java:140-152 underpin this). Without VirtualBoxingNode in the
  // Jeandle IR we instead key on the BoxedPrimitiveKind tag installed by
  ...
  // and the identity check cannot meaningfully proceed. Mirrors Graal's
  // PartialEscapeClosure.MergeProcessor.processPhi early-bail on a
  // self-referencing phi (java:1377-1380 region).
  for (unsigned i = 0; i < N; ++i) {
...
void Analyzer::applyThreeTier(Instruction *I) {
  ...
  // or the virtual was conservatively flipped ineligible); false if the base
  // isn't a virtual, in which case we fall through to the generic-escape
  // handling below.
  if (auto *RMW = dyn_cast<AtomicRMWInst>(I)) {
    if (tier2AtomicRMW(RMW))
      return;
    // Not on a virtual base: fall through to generic-escape path.
  }
  if (auto *CX = dyn_cast<AtomicCmpXchgInst>(I)) {
    if (tier2CmpXchg(CX))
      return;
    // Not on a virtual base: fall through.
  }
  ...
  if (Aliases.hasVirtualInputs(I)) {
    ...
    // FieldStates) or a constant data array global (staged constant load).
    // If tier2ArrayCopy folds OR conservatively bails by flipping the dst
    // VO's eligibility, it returns true and we stop. If the dst pointer is
    // NOT a virtual at all, it returns false so we fall through and let a
    // virtual SOURCE escape via materializeAllVirtualOperands below.
    if (auto *MTI = dyn_cast<MemTransferInst>(I)) {
      if (tier2ArrayCopy(MTI))
        return;
    ...
    //
    // Expected behaviour, transcribed from Graal's virtualize:
    ...
    // loads from the materialized receiver.
    ...
    // place, so Phase 2 is technically actionable.
    ...
    //              undef (Graal semantics: the node is purely advisory).
    //
    ...
    if (auto *ICmp = dyn_cast<ICmpInst>(I)) {
      if (ICmp->isEquality()) {
        ...
        // comparison over the boxed primitives (Graal:
        // ObjectEqualsNode.java:157-195 routes through
        // VirtualBoxingNode.getBoxedValue when both inputs are boxed
        // virtuals of the same JavaKind).
        //
...
void Analyzer::tier1Allocate(CallBase *CB) {
  ...
  // values. Mirrors Graal PartialEscapeClosure lines 310-329 where, in
  // MATERIALIZE_ALL, a virtualizable node is virtualised AND immediately
  // ensureMaterialized before the next fixed node. The end-of-block
  // emission is the practical compromise: intra-block folds work,
  // end-of-block emission is dominance-safe.
  ...
  // user-visible semantics. Mirrors Graal's NewInstanceNode +
  // RegisterFinalizerNode interaction.
  ...
  // Mirrors Graal's MetaAccessExtensionProvider.canVirtualize.
...
bool Analyzer::tier2Store(StoreInst *SI) {
  ...
  // symbolic indices (force materialization, matching Graal's
  // StoreIndexedNode "index constant only" policy).
    ...
    if (auto *G = dyn_cast<GetElementPtrInst>(Ptr)) {
      if (auto Match = VObj.matchArrayElementGEP(G, DL)) {
        if (auto *CI = dyn_cast<ConstantInt>(Match->Index)) {
          ...
          // Symbolic index — Graal bails here; we materialize.
          Eligible[*BaseID] = false;
          return true;
        }
      }
  ...
  // entries the front-end already stored). Mirrors Graal's
  // VirtualArrayNode.virtualizeByteArrayWrite.
  Type *I8Ty = Type::getInt8Ty(F.getContext());
  if (Val->getType()->isIntegerTy() && VObj.isArray() &&
      VObj.ArrayElementType == I8Ty) {
    uint64_t ValBits = DL.getTypeSizeInBits(Val->getType());
    if (ValBits > 8 && (ValBits % 8) == 0) {
      unsigned NBytes = static_cast<unsigned>(ValBits / 8);
      // Validate range fits within the array.
      int64_t LastOff = *Offset + static_cast<int64_t>(NBytes) - 1;
      if (LastOff < static_cast<int64_t>(VObj.ArrayBaseOffset) ||
          static_cast<uint64_t>(LastOff) >=
              VObj.ArrayBaseOffset + VObj.ArrayLength) {
        Eligible[*BaseID] = false;
        return true;
      }
      // Each per-byte slot must validate as an i8 entry. Bail on any
      // overlap conflict (e.g. an existing wider non-i8 field that crosses
      // this range).
      for (unsigned i = 0; i < NBytes; ++i) {
        if (VObj.getOrCreateFieldIndex(*Offset + static_cast<int64_t>(i),
                                       I8Ty) < 0) {
          Eligible[*BaseID] = false;
          return true;
        }
      }
      // Decompose. For a constant Val we precompute per-byte ConstantInts;
      // otherwise we emit unparented `trunc i8 (lshr Val, i*8)` instructions
      // and let any subsequent reassembly load splice them via OwnedInsts.
      auto *ValC = dyn_cast<Constant>(Val);
      Type *ValTy = Val->getType();
      auto &FS = FieldStates[*BaseID];
      for (unsigned i = 0; i < NBytes; ++i) {
        Value *Byte = nullptr;
        if (ValC) {
          // Constant decomposition via ConstantFoldBinaryOpOperands +
          // ConstantExpr::getTrunc.
          Constant *Sh = ConstantInt::get(ValTy, i * 8ULL);
          Constant *Shifted = ConstantFoldBinaryOpOperands(
              Instruction::LShr, ValC, Sh, DL);
          if (!Shifted) {
            Eligible[*BaseID] = false;
            return true;
          }
          Byte = ConstantExpr::getTrunc(Shifted, I8Ty);
        } else {
          Value *Src = Val;
          if (i > 0) {
            Constant *ShAmt = ConstantInt::get(ValTy, i * 8ULL);
            Instruction *Shr = BinaryOperator::Create(
                Instruction::LShr, Val, ShAmt, "pea.bytestore.shr",
                /*InsertBefore=*/nullptr);
            Shr->setDebugLoc(SI->getDebugLoc());
            Result.OwnedInsts.emplace_back(Shr);
            Src = Shr;
          }
          Instruction *Tr = CastInst::Create(
              Instruction::Trunc, Src, I8Ty, "pea.bytestore.tr",
              /*InsertBefore=*/nullptr);
          Tr->setDebugLoc(SI->getDebugLoc());
          Result.OwnedInsts.emplace_back(Tr);
          Byte = Tr;
        }
        FS[*Offset + static_cast<int64_t>(i)] = jeandle::FieldValue::scalar(Byte);
        FS[*Offset + static_cast<int64_t>(i)].setDeclaredType(I8Ty);
      }
      jeandle::PEAResult::Effect E;
...
void Analyzer::tier2Load(LoadInst *LI) {
  ...
  // being contained), we bail — Graal would need multi-slot read+concat.
  ...
  if (OverlapsNoncontained) {
    // Byte-array reassembly. Mirrors Graal's
    // VirtualArrayNode.virtualizeByteArrayRead: when the load straddles
    // multiple narrower slots that the analyzer was forced to track at the
    // byte granularity (because the front-end stored byte-by-byte via
    // Unsafe.putByte), recompose the wider integer load from the
    // contributing byte FieldStates. Little-endian only.
    //
    // Preconditions:
    //   - load is an integer of width N*8 (N = LoadByteSize)
    //   - VObj is an array with i8 element type
    //   - every byte slot [Offset .. Offset+LoadByteSize) is either an
    //     existing scalar i8 FieldStates entry or absent (default zero)
    //   - no slot is wider than 1 byte (i.e. the OverlapsNoncontained came
    //     from a byte-decomposed array; if a wider entry sits underneath,
    //     bail conservatively — see the inverse path that
    //     keeps everything byte-wide)
    Type *I8Ty = Type::getInt8Ty(F.getContext());
    bool TryReassemble = LoadTy->isIntegerTy() &&
                         (LoadBits % 8) == 0 && LoadByteSize > 0 &&
                         VObj.isArray() && VObj.ArrayElementType == I8Ty;
    if (TryReassemble) {
      // Confirm every overlapping Field is exactly 1 byte wide and lies
      // fully within [Offset, LoadEnd).
      for (const auto &Fld : VObj.Fields) {
        int64_t FEnd = Fld.Offset + static_cast<int64_t>(Fld.ByteSize);
        if (FEnd <= *Offset)
          continue;
        if (Fld.Offset >= LoadEnd)
          break;
        if (Fld.ByteSize != 1) {
          TryReassemble = false;
          break;
    ...
    if (TryReassemble) {
      auto FSIt = FieldStates.find(*BaseID);
      // Collect per-byte values (constant or SSA). Index i corresponds to
      // byte at Offset + i.
      SmallVector<Value *, 8> Bytes(LoadByteSize, nullptr);
      bool AllConstant = true;
      bool Bailed = false;
      for (uint8_t i = 0; i < LoadByteSize; ++i) {
        int64_t Off = *Offset + static_cast<int64_t>(i);
        const jeandle::FieldValue *Slot = nullptr;
        if (FSIt != FieldStates.end()) {
          auto It2 = FSIt->second.find(Off);
          if (It2 != FSIt->second.end())
            Slot = &It2->second;
        }
        if (!Slot || Slot->isUnknown()) {
          // Default i8 zero.
          Bytes[i] = ConstantInt::get(I8Ty, 0);
          continue;
        }
        if (!Slot->isScalar()) {
          // VirtualRef / MaterializedRef in a byte slot is exotic; bail.
          Bailed = true;
          break;
        }
        Value *V = Slot->getScalar();
        if (V->getType() != I8Ty) {
          // Slot was recorded with a wider declared type (shouldn't happen
          // when ByteSize == 1 at this point, but defend).
          Bailed = true;
          break;
        }
        Bytes[i] = V;
      ...
      if (!Bailed) {
        Value *Reassembled = nullptr;
        if (AllConstant) {
          // Compose into a single ConstantInt little-endian.
          APInt Acc(LoadBits, 0);
          for (uint8_t i = 0; i < LoadByteSize; ++i) {
            auto *CI = cast<ConstantInt>(Bytes[i]);
            APInt B = CI->getValue().zext(LoadBits);
            B <<= (i * 8);
            Acc |= B;
          }
          Reassembled = ConstantInt::get(LoadTy, Acc);
        } else {
          // Build `or (shl (zext byte_i, LoadTy), i*8) ...` chain. Order
          // doesn't matter for correctness; keep ascending so the IR reads
          // naturally low-to-high.
          Value *Acc = nullptr;
          for (uint8_t i = 0; i < LoadByteSize; ++i) {
            // Zero-extend byte to LoadTy.
            Value *Z = nullptr;
            if (auto *CI = dyn_cast<ConstantInt>(Bytes[i])) {
              Z = ConstantInt::get(LoadTy, CI->getValue().zext(LoadBits));
            } else {
              Instruction *ZE = CastInst::Create(
                  Instruction::ZExt, Bytes[i], LoadTy, "pea.bytemerge.zext",
                  /*InsertBefore=*/nullptr);
              ZE->setDebugLoc(LI->getDebugLoc());
              Result.OwnedInsts.emplace_back(ZE);
              Z = ZE;
            }
            // Shift left by i*8.
            Value *Shifted = Z;
            if (i > 0) {
              if (auto *CZ = dyn_cast<Constant>(Z)) {
                Constant *Folded = ConstantFoldBinaryOpOperands(
                    Instruction::Shl, CZ, ConstantInt::get(LoadTy, i * 8), DL);
                if (Folded) {
                  Shifted = Folded;
                } else {
                  Constant *ShAmt = ConstantInt::get(LoadTy, i * 8);
                  Instruction *Sh = BinaryOperator::Create(
                      Instruction::Shl, Z, ShAmt, "pea.bytemerge.shl",
                      /*InsertBefore=*/nullptr);
                  Sh->setDebugLoc(LI->getDebugLoc());
                  Result.OwnedInsts.emplace_back(Sh);
                  Shifted = Sh;
                }
              } else {
                Constant *ShAmt = ConstantInt::get(LoadTy, i * 8);
            ...
            if (!Acc) {
              ...
              if (isa<Constant>(Acc) && isa<Constant>(Shifted)) {
                Constant *Folded = ConstantFoldBinaryOpOperands(
                    Instruction::Or, cast<Constant>(Acc),
                    cast<Constant>(Shifted), DL);
                if (Folded) {
                  Acc = Folded;
                } else {
                  Instruction *Or = BinaryOperator::Create(
                      Instruction::Or, Acc, Shifted, "pea.bytemerge.or",
                      /*InsertBefore=*/nullptr);
                  Or->setDebugLoc(LI->getDebugLoc());
                  Result.OwnedInsts.emplace_back(Or);
                  Acc = Or;
                }
              } else {
                Instruction *Or = BinaryOperator::Create(
        ...
        E.Target = LI;
        E.Replacement = Reassembled;
        E.SeqNo = Result.nextSeqNo();
        E.ObjID = *BaseID;
        Result.addBlockEffect(std::move(E));
        if (auto *RI = dyn_cast<Instruction>(Reassembled))
          (void)RI; // tracked via OwnedInsts already
        Aliases.addScalarAlias(LI, Reassembled);
        return;
      }
    }
    Eligible[*BaseID] = false;
  ...
  if (Existing->isVirtualRef()) {
    // Nested-virtual load: Graal's LoadFieldNode.virtualize simply calls
    // tool.getEntry, which returns the inner VirtualObjectNode unchanged —
    // i.e. loading a field whose tracked value is another virtual yields
    // that other virtual (still virtual!), and tool.replaceWith forwards
    // it. Match that here: forward the load to the inner virtual's
    // allocation Value and install a virtual alias from the load to
...
// other thread yet (it's still an unmaterialized abstract value living
// entirely within the analyzer's per-thread tracking), so no concurrent
// access can race the field. The atomic-op's "atomic" guarantee is
// trivially satisfied by single-threaded folding; no fence/memory-order
// translation is needed.
//
// Graal does NOT virtualize the AtomicReadAndAddNode / AtomicReadAndWriteNode
// — those nodes go straight to lowering — but AbstractUnsafeCompareAndSwapNode
// IS a Virtualizable in Graal (see AbstractUnsafeCompareAndSwapNode.virtualize
// at lines 117-189): for a constant-offset CAS on a virtual it folds the
// equality compare and updates the field entry. We replicate that for
// cmpxchg, and ALSO extend to atomicrmw (the LLVM equivalent of
// AtomicReadAndAddNode / AtomicReadAndWriteNode) — at the IR level the
// virtual-field fold for atomicrmw is just as safe as for CAS and avoids
// forcing materialization for VarHandle-style Java code patterns.

// Resolve a virtual base + constant offset + within-slot logic for an
// atomic op on Ptr. Returns true on success; the out-params describe the
// matching field-state slot (EntryOffset is the FieldStates key; the value
// at that key, if present, has DeclaredType matching the slot width).
// WithinSlotByteOff is the byte offset of the access *within* the slot.
//
// On any condition that would force conservative bail (non-virtual base,
// non-constant offset, straddling overlap), this sets Eligible[*BaseID] =
// false (where appropriate) and returns false. The caller treats false as
...
struct AtomicSlot {
  ...
  int64_t EntryOffset;
  int64_t WithinSlotByteOff;
  const jeandle::FieldValue *Existing; // null if no entry recorded
};
} // namespace

bool Analyzer::tier2AtomicRMW(AtomicRMWInst *RMW) {
  ...
  auto BaseID = jeandle::pea::resolveVirtualRef(Ptr, CurrentState, Aliases, DL);
  if (!BaseID)
    return false; // not on a virtual — fall through to generic-escape path.
  ...
  // Type-overlap validation, identical to tier2Store. A bogus access width on
  // a virtual flips eligibility.
  jeandle::VirtualObject &VObj = *Result.VirtualObjects[*BaseID];
  if (VObj.getOrCreateFieldIndex(*Offset, AccessTy) < 0) {
    Eligible[*BaseID] = false;
    return true;
  }
  ...
  for (const auto &F : VObj.Fields) {
    ...
    if (FEnd <= *Offset)
      continue;
    if (F.Offset >= AccessEnd)
      break;
    if (F.Offset <= *Offset && FEnd >= AccessEnd) {
      EntryOffset = F.Offset;
    } else {
      OverlapsNoncontained = true;
    }
  ...
  // Within-slot atomic op is unsupported for sub-slot accesses: would need
  // read-modify-write semantics on a wider slot with bit-level masking.
  // Conservative bail.
  if (WithinSlotByteOff != 0) {
    Eligible[*BaseID] = false;
    return true;
  ...
  // Determine the OLD value of the slot, matching tier2Load semantics:
  //   - missing/unknown entry → default-for(AccessTy) (zero for ints/floats,
  //     null for pointers).
  //   - scalar entry → the recorded Value*, coerced to AccessTy if needed.
  // A virtualRef or materializedRef entry means the slot currently holds a
  // pointer value; an atomicrmw on a pointer slot is exotic (Java doesn't
  // emit it directly), and the per-op fold below requires a primitive/value
  // operand, so bail conservatively.
  Value *Old = nullptr;
  if (!Existing || Existing->isUnknown()) {
    Old = jeandle::FieldValue::defaultFor(AccessTy);
  } else if (Existing->isScalar()) {
    Value *V = Existing->getScalar();
    Old = coerceToType(V, AccessTy, RMW, /*WithinSlotByteOff=*/0);
    if (!Old) {
      Eligible[*BaseID] = false;
      return true;
    }
  } else {
    // VirtualRef / MaterializedRef in a slot being atomically updated: bail.
    Eligible[*BaseID] = false;
    return true;
  }

  // Compute NewEntry from OldValue and Val per the atomic op kind. We
  // restrict to ops that have a defined IR-level lowering and that match
  // Java's VarHandle vocabulary (xchg, add, sub, and, or, xor) plus the
  // floating-point reduction ops (fadd, fsub, fmin, fmax) used by VarHandle
  // for double/float. Other ops (nand, signed/unsigned min/max,
  // increment/decrement wrap) are conservatively bailed: they could be
  // supported with more plumbing, but Java sources don't emit them at
  // constant-offset against a pure virtual today.
  AtomicRMWInst::BinOp Op = RMW->getOperation();
  Constant *OldC = dyn_cast<Constant>(Old);
  Constant *ValC = dyn_cast<Constant>(Val);

  Value *NewEntryV = nullptr;
  Instruction::BinaryOps IROp = static_cast<Instruction::BinaryOps>(0);
  bool HasBinOp = true;
  bool IsFP = false;
  switch (Op) {
  case AtomicRMWInst::Xchg:
    // NewEntry = Val (any value).
    NewEntryV = Val;
    HasBinOp = false;
    break;
  case AtomicRMWInst::Add:
    IROp = Instruction::Add;
    break;
  case AtomicRMWInst::Sub:
    IROp = Instruction::Sub;
    break;
  case AtomicRMWInst::And:
    IROp = Instruction::And;
    break;
  case AtomicRMWInst::Or:
    IROp = Instruction::Or;
    break;
  case AtomicRMWInst::Xor:
    IROp = Instruction::Xor;
    break;
  // FP reductions. We restrict to constant ValC (and constant Old)
  // because there's no general FP "Conditional fold" analogue to the
  // CAS path: emitting a non-constant FP rmw would require a fadd/fsub/etc.
  // BinaryOperator. We avoid that complexity (it adds little value over the
  // bail since Java VarHandle FP rmw on a pure virtual is rare) — bail on
  // non-constant ValC.
  case AtomicRMWInst::FAdd:
    IROp = Instruction::FAdd;
    IsFP = true;
    break;
  case AtomicRMWInst::FSub:
    IROp = Instruction::FSub;
    IsFP = true;
    break;
  case AtomicRMWInst::FMin:
  case AtomicRMWInst::FMax:
    // ConstantFoldBinaryOpOperands handles minnum/maxnum via the corresponding
    // intrinsic constant fold path; we route them through
    // simplifyBinOp-equivalent ConstantFolding by lowering to ICmp-style
    // select-of-fcmp. Easiest: bail when non-constant; otherwise compute
    // the fold via ConstantFP comparison.
    IsFP = true;
    break;
  default:
    // Nand / [U]Min / [U]Max / UIncWrap / UDecWrap: not in the supported
    // set. Bail.
    Eligible[*BaseID] = false;
    return true;
  }
  ...
  if (HasBinOp) {
    if (IsFP) {
      // FP path: require both OldC and ValC to be constants. AccessTy must be
      // FP; defensive check.
      ...
      if (Op == AtomicRMWInst::FAdd || Op == AtomicRMWInst::FSub) {
        NewEntryV = ConstantFoldBinaryOpOperands(IROp, OldC, ValC, DL);
        if (!NewEntryV) {
          Eligible[*BaseID] = false;
          return true;
        }
      } else {
        // FMin / FMax: compute via ConstantFP. atomicrmw fmin/fmax follow
        // IEEE-754 minNum/maxNum semantics (NaN-quiet propagation: if one
        // operand is NaN, the other wins; if both NaN, the result is NaN).
        auto *OldFP = dyn_cast<ConstantFP>(OldC);
        auto *ValFP = dyn_cast<ConstantFP>(ValC);
        if (!OldFP || !ValFP) {
          Eligible[*BaseID] = false;
          return true;
        }
        APFloat OldAPF = OldFP->getValueAPF();
        APFloat ValAPF = ValFP->getValueAPF();
        APFloat ResAPF = (Op == AtomicRMWInst::FMin)
                             ? minnum(OldAPF, ValAPF)
                             : maxnum(OldAPF, ValAPF);
        NewEntryV = ConstantFP::get(AccessTy->getContext(), ResAPF);
      }
    } else if (!AccessTy->isIntegerTy()) {
      // The integer binops above are only well-defined on integer types.
      Eligible[*BaseID] = false;
      return true;
    } else if (OldC && ValC) {
      // Compile-time fold. ConstantFoldBinaryOpOperands handles And/Or/etc.
      // that ConstantExpr::get no longer supports in recent LLVM.
      NewEntryV = ConstantFoldBinaryOpOperands(IROp, OldC, ValC, DL);
      if (!NewEntryV) {
        // Defensive: shouldn't happen for the integer binops we support.
        Eligible[*BaseID] = false;
        return true;
      }
    } else {
      // Emit an unparented binop into OwnedInsts. The transform's ReplaceLoad
      // handler walks the operand chain in postorder to splice unparented
      // operand instructions immediately before the Target. NewEntry is the
      // updated field-state, NOT the replacement for the atomicrmw itself
      // (Old is the replacement), so we must explicitly register NewEntry
      // with OwnedInsts and emit a separate splice for it. We do this by
      // appending a ReplaceInput-style effect? No — the cleanest path is to
      // park the inst as part of the ReplaceLoad chain: if NewEntry happens
      // to reference Old (which it does for binop forms), and Old is the
      // Replacement, the OwnedInsts walk wouldn't pick up NewEntry because
      // NewEntry is not in Old's operand transitive closure (Old is in
      // NewEntry's).
      //
      // The simpler choice: NewEntry only needs to survive in IR if it is
      // used by something downstream — which only happens via a subsequent
      // load that reads this slot, and that load's ReplaceLoad effect will
      // RAUW to NewEntry and walk NewEntry's operand chain (which includes
      // Old, a constant or an existing-in-IR Value*, and Val, similarly).
      // So we register NewEntry in OwnedInsts and let any future
      // ReplaceLoad targeting it splice it in. Defensive: until someone
      // actually consumes NewEntry, it stays unparented and is owned by
      // PEAResult::OwnedInsts (which cleans up on teardown).
      Instruction *NewI =
          BinaryOperator::Create(IROp, Old, Val, "pea.atomicrmw",
                                 /*InsertBefore=*/nullptr);
      NewI->setDebugLoc(RMW->getDebugLoc());
      Result.OwnedInsts.emplace_back(NewI);
      NewEntryV = NewI;
    }
  }

  // Update field state.
  if (auto *NewC = dyn_cast<Constant>(NewEntryV)) {
    FieldStates[*BaseID][EntryOffset] = jeandle::FieldValue::scalar(NewC);
  } else {
  ...
  // ReplaceLoad apply path does generic RAUW + erase on the Target, which
  // is exactly what we need; reusing the existing kind keeps the transform
  // simple.
  jeandle::PEAResult::Effect E;
  E.Kind = jeandle::PEAResult::EffectKind::ReplaceLoad;
  E.Block = RMW->getParent();
  E.Target = RMW;
  E.Replacement = Old;
  E.SeqNo = Result.nextSeqNo();
  E.ObjID = *BaseID;
  ...
  // Scalar-alias the atomicrmw to Old so downstream resolveVirtualRef /
  // pointer-derivation queries (rare for the result of an atomicrmw, but
  // possible if Java code feeds it into an ICmp/store) see through.
  Aliases.addScalarAlias(RMW, Old);
  return true;
}

bool Analyzer::tier2CmpXchg(AtomicCmpXchgInst *CX) {
  Value *Ptr = CX->getPointerOperand();
  Value *Expected = CX->getCompareOperand();
  Value *NewVal = CX->getNewValOperand();
  Type *AccessTy = Expected->getType();
  ...
  if (!Existing || Existing->isUnknown()) {
    Current = jeandle::FieldValue::defaultFor(AccessTy);
  } else if (Existing->isScalar()) {
    Value *V = Existing->getScalar();
    Current = coerceToType(V, AccessTy, CX, /*WithinSlotByteOff=*/0);
    if (!Current) {
      Eligible[*BaseID] = false;
      return true;
    }
  } else {
    // pointer-typed slot via VirtualRef / MaterializedRef: bail.
    Eligible[*BaseID] = false;
    return true;
  }

  Constant *CurrentC = dyn_cast<Constant>(Current);
  Constant *ExpectedC = dyn_cast<Constant>(Expected);

  // The cmpxchg result is `{T, i1}` — the prior value of the slot and the
  // success flag (equal? Expected == prior).
  StructType *ResultTy = cast<StructType>(CX->getType());
  Type *I1Ty = ResultTy->getElementType(1);

  // When both Current and Expected are constants, fold equality
  // statically (the existing fast path). Otherwise, replicate Graal's
  // AbstractUnsafeCompareAndSwapNode virtualizer "Unknown outcome" branch
  // (AbstractUnsafeCompareAndSwapNode.java:117-189): build a Conditional
  // node `select(eq(Current, Expected), NewVal, Current)` for the slot's
  // new value and a corresponding {ty, i1} struct for the cmpxchg result.
  // Float CAS: bail. Graal's Conditional fold has a known latent bug for
  // FP types (FP equality is non-reflexive for NaN); LLVM cmpxchg on FP is
  // illegal anyway (cmpxchg requires integer or pointer).
  bool BothConst = CurrentC && ExpectedC;
  if (!BothConst && AccessTy->isFloatingPointTy()) {
    Eligible[*BaseID] = false;
    return true;
  }

  if (BothConst) {
    // For pointer types, equality folding requires both pointer-typed
    // constants. ConstantExpr::getICmp handles ConstantPointerNull and other
    // pointer constants for ICMP_EQ.
    Constant *EqC = ConstantFoldCompareInstOperands(
        ICmpInst::ICMP_EQ, CurrentC, ExpectedC, DL);
    ConstantInt *EqInt = dyn_cast_or_null<ConstantInt>(EqC);
    if (!EqInt) {
      Eligible[*BaseID] = false;
      return true;
    }
    bool Equal = !EqInt->isZero();

    // If equal: the slot is updated to NewVal. NewVal may itself be
    // non-constant, in which case we record the SSA value as the new field
    // entry (mirroring tier2Store's scalar path).
    if (Equal) {
      if (auto *NewValC = dyn_cast<Constant>(NewVal)) {
        FieldStates[*BaseID][EntryOffset] = jeandle::FieldValue::scalar(NewValC);
      } else {
        FieldStates[*BaseID][EntryOffset] = jeandle::FieldValue::scalar(NewVal);
      }
      FieldStates[*BaseID][EntryOffset].setDeclaredType(AccessTy);
    }
    // Else: no state change, slot retains Current.

    // Build the replacement struct `{Current, Equal-as-i1}`. Both elements
    // are constants here (Current is CurrentC; Equal is a constant bool),
    // so ConstantStruct::get yields a Constant.
    Constant *EqBool = ConstantInt::get(I1Ty, Equal ? 1 : 0);
    Constant *ReplStruct = ConstantStruct::get(ResultTy, {CurrentC, EqBool});

    jeandle::PEAResult::Effect E;
    E.Kind = jeandle::PEAResult::EffectKind::ReplaceLoad;
    E.Block = CX->getParent();
    E.Target = CX;
    E.Replacement = ReplStruct;
    E.SeqNo = Result.nextSeqNo();
    E.ObjID = *BaseID;
    Result.addBlockEffect(std::move(E));

    // No scalar-alias install: a struct-typed cmpxchg result isn't a
    // candidate for resolveVirtualRef (which expects a pointer).
    return true;
  }

  // Non-constant path. Build:
  //   %eq    = icmp eq <AccessTy> Current, Expected
  //   %slot' = select i1 %eq, <AccessTy> NewVal, <AccessTy> Current
  //   %s0    = insertvalue {<AccessTy>, i1} poison, <AccessTy> Current, 0
  //   %s1    = insertvalue {<AccessTy>, i1} %s0,    i1 %eq,             1
  // and:
  //   - update FieldStates[*BaseID][EntryOffset] = scalar(%slot')
  //   - emit ReplaceLoad effect with Replacement = %s1
  // The transform's ReplaceLoad handler walks Replacement's operand graph
  // in postorder and splices unparented operands before the Target, so the
  // entire chain lands in CX's basic block immediately before CX.
  //
  // Type sanity: cmpxchg requires AccessTy to be an integer or pointer
  // type at the IR level (LLVM verifier rejects FP/struct/aggregate). Use
  // ICmpInst (integer/pointer compare); ICMP_EQ handles both kinds.
  if (!AccessTy->isIntegerTy() && !AccessTy->isPointerTy()) {
    Eligible[*BaseID] = false;
    return true;
  }

  Instruction *EqInst = ICmpInst::Create(
      Instruction::ICmp, ICmpInst::ICMP_EQ, Current, Expected, "pea.cas.eq",
      /*InsertBefore=*/static_cast<Instruction *>(nullptr));
  EqInst->setDebugLoc(CX->getDebugLoc());
  Result.OwnedInsts.emplace_back(EqInst);

  Instruction *SelInst = SelectInst::Create(EqInst, NewVal, Current,
                                            "pea.cas.sel",
                                            /*InsertBefore=*/nullptr);
  SelInst->setDebugLoc(CX->getDebugLoc());
  Result.OwnedInsts.emplace_back(SelInst);

  Value *Poison = PoisonValue::get(ResultTy);
  Instruction *S0 = InsertValueInst::Create(Poison, Current, {0}, "pea.cas.s0",
                                            /*InsertBefore=*/nullptr);
  S0->setDebugLoc(CX->getDebugLoc());
  Result.OwnedInsts.emplace_back(S0);
  Instruction *S1 = InsertValueInst::Create(S0, EqInst, {1}, "pea.cas.s1",
                                            /*InsertBefore=*/nullptr);
  S1->setDebugLoc(CX->getDebugLoc());
  Result.OwnedInsts.emplace_back(S1);

  FieldStates[*BaseID][EntryOffset] = jeandle::FieldValue::scalar(SelInst);
  FieldStates[*BaseID][EntryOffset].setDeclaredType(AccessTy);
...
bool Analyzer::foldMonitorEnter(CallBase *CB) {
  ...
  // point without its sibling's lock on the stack. Mirrors Graal's
  // materializeVirtualLocksBefore (PartialEscapeClosure.java:641-652) which
  // forces every sibling holding a shallower-depth lock to materialise
  // BEFORE the new virtual lock is added.
  //
...
bool Analyzer::foldArrayStoreCheck(CallBase *CB) {
  ...
  if (ElementKlass == 0) {
    ...
    // primitive arrays. Unconditionally elide. Mirrors Graal's
    // StoreIndexedNode.virtualize line 119: `elementKind.isPrimitive()`.
    Constant *True = ConstantInt::getTrue(CB->getType());
...
bool Analyzer::foldCheckIfValueBased(CallBase *CB) {
  ...
  // Provably non-value-based: fold the check to false. Mirrors Graal's
  // canonical folding of a value-based query against an exact (virtualized)
  // klass that does not carry the ValueBased marker.
...
//   - dst must alias a virtual ARRAY (instance virtual dst -> bail).
//   - length must be constant and an exact multiple of dst.ArrayIndexScale.
//   - dst offset (relative to dst.ArrayBaseOffset) must be a multiple of
//     dst.ArrayIndexScale (so it names a whole element boundary).
//   - src kinds accepted:
//       (a) another virtual array with the same ArrayElementType and
//           ArrayIndexScale: per-slot copy from src.FieldStates.
//       (b) a constant global whose initializer is ConstantDataSequential
//           (i.e. constant primitive array data) or ConstantAggregateZero
//           (zero-filled): staged constant load into each dst slot.
//     Anything else (parameter, opaque load, non-array virtual, type mismatch,
//     out-of-bounds src/dst, symbolic src offset) -> bail.
//   - Overlap (memmove): values are first BUFFERED out of the source side
//     into a SmallVector and only then written into dst.FieldStates, which
//     makes the per-slot update sequence overlap-safe in both same-array and
//     different-array forms. (Note: llvm.memcpy on overlapping ranges is UB
//     anyway, so we don't have to be cleverer than that; for memmove this
//     matches the spec exactly.)
//
// Return value: true iff the call was successfully folded (per-slot transfer
// + EliminateStore emitted). False in every bail case so the caller's
// generic-escape path (materializeAllVirtualOperands) runs. The caller path
// is responsible for ensuring that a virtual dst we couldn't fold gets
// materialized at the memcpy (so the memcpy's surviving operand sees a real
// pointer rather than an erased / poisoned virtual alloc).
bool Analyzer::tier2ArrayCopy(MemTransferInst *MI) {
  using jeandle::FieldValue;
  using jeandle::ObjectID;
  using jeandle::pea::resolveVirtualRef;
  using jeandle::pea::stripPointerCastsAndOffsets;

  Value *Dst = MI->getRawDest();
  Value *Src = MI->getRawSource();
  Value *Len = MI->getLength();

  // Resolve dst to a virtual object. If dst isn't a virtual, leave the
  // memcpy alone (caller's materializeAllVirtualOperands handles a virtual
  // src if any).
  auto DstID = resolveVirtualRef(Dst, CurrentState, Aliases, DL);
  if (!DstID)
    return false;

  jeandle::VirtualObject &DstVO = *Result.VirtualObjects[*DstID];

  // Volatile memcpy / memmove carries observable memory-model semantics.
  // Conservatively bail.
  if (MI->isVolatile())
    return false;
  ...
  if (!DstVO.isArray() || !DstVO.ArrayElementType ||
    ...
    return false;
  const int64_t Scale = static_cast<int64_t>(DstVO.ArrayIndexScale);

  // Length must be a constant. Symbolic length -> bail (Graal requires
  // replacedLength.isConstant() too — BasicArrayCopyNode.java:293).
  auto *LenCI = dyn_cast<ConstantInt>(Len);
  if (!LenCI)
    return false;
  uint64_t LenBytes = LenCI->getZExtValue();
  if (LenBytes == 0) {
    // No-op copy: just remove the call. Don't touch FieldStates.
    jeandle::PEAResult::Effect E;
    E.Kind = jeandle::PEAResult::EffectKind::EliminateStore;
    E.Block = MI->getParent();
    E.Target = MI;
    E.SeqNo = Result.nextSeqNo();
    E.ObjID = *DstID;
    Result.addBlockEffect(std::move(E));
  ...
  if (LenBytes % static_cast<uint64_t>(Scale) != 0)
    return false;
  uint64_t Count = LenBytes / static_cast<uint64_t>(Scale);

  // Helper: resolve a pointer to its element offset within the named virtual
  // array. Tries first the constant-byte-offset path via
  // stripPointerCastsAndOffsets (covers typed-element GEPs whose index is
  // constant-folded by the front-end). Falls back to matchArrayElementGEP for
  // symbolic indices on the outermost GEP. Returns -1 on bail.
  auto resolveElementOffset = [&](Value *Ptr,
                                  const jeandle::VirtualObject &VO) -> int64_t {
    int64_t Off = 0;
    bool NonConst = false;
    (void)stripPointerCastsAndOffsets(Ptr, DL, &Off, &NonConst);
    if (!NonConst) {
      int64_t Adj = Off - static_cast<int64_t>(VO.ArrayBaseOffset);
      if (Adj < 0)
        return -1;
      if (Adj % static_cast<int64_t>(VO.ArrayIndexScale) != 0)
        return -1;
      return Adj / static_cast<int64_t>(VO.ArrayIndexScale);
    }
    if (auto *G = dyn_cast<GetElementPtrInst>(Ptr)) {
      if (auto Match = VO.matchArrayElementGEP(G, DL)) {
        if (auto *CI = dyn_cast<ConstantInt>(Match->Index)) {
          int64_t Cidx = CI->getSExtValue();
          if (Cidx < 0)
            return -1;
  ...
  int64_t DstElemOff = resolveElementOffset(Dst, DstVO);
  if (DstElemOff < 0 ||
      static_cast<uint64_t>(DstElemOff) + Count >
          static_cast<uint64_t>(DstVO.ArrayLength))
    return false;

  // Now resolve the source. Three forms: virtual array, constant-data global,
  // anything else (bail).
  enum SrcKind { SK_Virtual, SK_ConstGlobal };
  SrcKind SK;
  int64_t SrcElemOff = 0;
  ObjectID SrcVOID = jeandle::InvalidObjectID;
  const jeandle::VirtualObject *SrcVO = nullptr;

  // For SK_ConstGlobal:
  ConstantDataSequential *SrcCDS = nullptr;
  bool SrcIsZero = false;

  auto SrcResolved = resolveVirtualRef(Src, CurrentState, Aliases, DL);
  if (SrcResolved) {
    SrcVOID = *SrcResolved;
    SrcVO = Result.VirtualObjects[SrcVOID].get();
    // Compatibility: must be an array with matching element type / scale.
    if (!SrcVO->isArray() || !SrcVO->ArrayElementType ||
        SrcVO->ArrayElementType != DstVO.ArrayElementType ||
        SrcVO->ArrayIndexScale != DstVO.ArrayIndexScale)
      return false;
    SrcElemOff = resolveElementOffset(Src, *SrcVO);
    if (SrcElemOff < 0 ||
        static_cast<uint64_t>(SrcElemOff) + Count >
            static_cast<uint64_t>(SrcVO->ArrayLength))
      return false;
    SK = SK_Virtual;
  } else {
    // Try the constant-global form: src is a constant GEP / cast chain rooted
    // at a `@x = constant [N x ElemTy] {...}` whose initializer is a
    // ConstantDataSequential (primitive data) or a zeroinitializer.
    int64_t SrcByteOff = 0;
    bool NonConst = false;
    Value *SrcRoot = stripPointerCastsAndOffsets(Src, DL, &SrcByteOff,
                                                  &NonConst);
    if (NonConst || SrcByteOff < 0)
      return false;
    auto *GV = dyn_cast_or_null<GlobalVariable>(SrcRoot);
    if (!GV || !GV->isConstant() || !GV->hasInitializer())
      return false;
    Constant *Init = GV->getInitializer();
    Type *SrcElemTy = nullptr;
    uint64_t SrcNumElems = 0;
    if (auto *CDS = dyn_cast<ConstantDataSequential>(Init)) {
      SrcCDS = CDS;
      SrcElemTy = CDS->getElementType();
      SrcNumElems = CDS->getNumElements();
    } else if (isa<ConstantAggregateZero>(Init)) {
      SrcIsZero = true;
      if (auto *AT = dyn_cast<ArrayType>(Init->getType())) {
        SrcElemTy = AT->getElementType();
        SrcNumElems = AT->getNumElements();
      } else {
        return false;
      }
    } else {
    ...
    if (SrcElemTy != DstVO.ArrayElementType)
      return false;
    if (SrcByteOff % Scale != 0)
      return false;
    SrcElemOff = SrcByteOff / Scale;
    if (SrcElemOff < 0 ||
        static_cast<uint64_t>(SrcElemOff) + Count > SrcNumElems)
      return false;
    SK = SK_ConstGlobal;
  }

  // Buffer the source-side FieldValues first, then write them into dst's
  // FieldStates. This sidesteps overlap concerns in both same-array and
  // different-array forms (memmove backward-copy semantics fall out for free).
  SmallVector<FieldValue, 16> SrcEntries;
  SrcEntries.reserve(Count);

  if (SK == SK_Virtual) {
    auto FIt = FieldStates.find(SrcVOID);
    for (uint64_t i = 0; i < Count; ++i) {
      int64_t SrcOffBytes = static_cast<int64_t>(SrcVO->ArrayBaseOffset) +
                            (SrcElemOff + static_cast<int64_t>(i)) * Scale;
      const FieldValue *Existing = nullptr;
      if (FIt != FieldStates.end()) {
        auto It2 = FIt->second.find(SrcOffBytes);
        if (It2 != FIt->second.end())
          Existing = &It2->second;
      }
      if (Existing && !Existing->isUnknown()) {
        SrcEntries.push_back(*Existing);
      } else {
        // Default value for a never-written element. Use the element type as
        // the FieldValue type (matches Java's default-zero initialisation).
        Constant *Def = FieldValue::defaultFor(DstVO.ArrayElementType);
        SrcEntries.push_back(FieldValue::scalar(Def));
      }
    }
  } else {
    // SK_ConstGlobal
    for (uint64_t i = 0; i < Count; ++i) {
      Constant *C = nullptr;
      if (SrcIsZero) {
        C = Constant::getNullValue(DstVO.ArrayElementType);
      } else {
        assert(SrcCDS && "expected ConstantDataSequential for non-zero source");
        C = SrcCDS->getElementAsConstant(SrcElemOff + i);
      }
      if (!C)
        return false;
      SrcEntries.push_back(FieldValue::scalar(C));
    }
  }

  // Validate type-overlap on every dst slot before mutating FieldStates, so a
  // mid-loop bail leaves the recorded state consistent. getOrCreateFieldIndex
  // may insert FieldDesc entries — that's fine: they describe the dst
  // virtual's element layout (which we know to be uniform).
  for (uint64_t i = 0; i < Count; ++i) {
    int64_t DstOffBytes = static_cast<int64_t>(DstVO.ArrayBaseOffset) +
                          (DstElemOff + static_cast<int64_t>(i)) * Scale;
    if (DstVO.getOrCreateFieldIndex(DstOffBytes, DstVO.ArrayElementType) < 0)
      return false;
  }

  // Apply the per-slot writes to dst.FieldStates.
  auto &DstFS = FieldStates[*DstID];
  for (uint64_t i = 0; i < Count; ++i) {
    int64_t DstOffBytes = static_cast<int64_t>(DstVO.ArrayBaseOffset) +
                          (DstElemOff + static_cast<int64_t>(i)) * Scale;
    DstFS[DstOffBytes] = SrcEntries[i];
  }

  // Emit EliminateStore on the memcpy itself.
  jeandle::PEAResult::Effect E;
  E.Kind = jeandle::PEAResult::EffectKind::EliminateStore;
  E.Block = MI->getParent();
  E.Target = MI;
...
// llvm.memset on a virtual array. Replicates the source byte
// across each element width and writes a per-element FieldStates entry.
// Only constant length and constant value are supported (matching Graal's
// virtualizer policy for memset). Volatile bails. Element-aligned access
// only — anything non-aligned bails to materialize.
bool Analyzer::tier2MemSet(MemSetInst *MSI) {
  using jeandle::FieldValue;
  using jeandle::ObjectID;
  using jeandle::pea::resolveVirtualRef;
  using jeandle::pea::stripPointerCastsAndOffsets;

  Value *Dst = MSI->getRawDest();
  Value *Val = MSI->getValue();   // i8
  ...
  if (!ValCI)
    return false;
  // ValCI is always i8 (intrinsic signature).
  uint8_t ByteVal = static_cast<uint8_t>(ValCI->getZExtValue() & 0xFF);

  // Resolve dst element offset.
  int64_t DstByteOff = 0;
  bool NonConst = false;
  (void)stripPointerCastsAndOffsets(Dst, DL, &DstByteOff, &NonConst);
  int64_t DstElemOff = -1;
  if (!NonConst) {
    int64_t Adj = DstByteOff - static_cast<int64_t>(DstVO.ArrayBaseOffset);
    if (Adj >= 0 && Adj % Scale == 0)
      DstElemOff = Adj / Scale;
  }
  if (DstElemOff < 0) {
    if (auto *G = dyn_cast<GetElementPtrInst>(Dst)) {
      if (auto Match = DstVO.matchArrayElementGEP(G, DL)) {
        if (auto *CI = dyn_cast<ConstantInt>(Match->Index)) {
          int64_t Cidx = CI->getSExtValue();
          if (Cidx >= 0)
            DstElemOff = Cidx;
  ...
  // Build the per-element constant by replicating ByteVal across the element
  // width. For pointer element types (reference array), only ByteVal == 0
  // (i.e. memset-zero-init) makes sense; any non-zero would synthesize a
  // bogus pointer constant. Bail otherwise.
  Type *ElemTy = DstVO.ArrayElementType;
  Constant *PerElement = nullptr;
  if (ElemTy->isIntegerTy()) {
    uint64_t Bits = DL.getTypeSizeInBits(ElemTy);
    if (Bits == 0 || (Bits % 8) != 0)
      return false;
    APInt Acc(Bits, 0);
    for (uint64_t i = 0; i < Bits / 8; ++i) {
      APInt B(Bits, ByteVal);
      B <<= (i * 8);
      Acc |= B;
    }
    PerElement = ConstantInt::get(ElemTy, Acc);
    ...
    PerElement = ConstantFP::get(ElemTy->getContext(),
                                 APFloat(ElemTy->getFltSemantics(), Acc));
  } else if (ElemTy->isPointerTy()) {
    if (ByteVal != 0)
      return false;
    PerElement = Constant::getNullValue(ElemTy);
  } else {
    return false;
  }
  ...
  for (uint64_t i = 0; i < Count; ++i) {
    int64_t DstOffBytes = static_cast<int64_t>(DstVO.ArrayBaseOffset) +
                          (DstElemOff + static_cast<int64_t>(i)) * Scale;
    if (DstVO.getOrCreateFieldIndex(DstOffBytes, ElemTy) < 0)
      return false;
  }
  ...
  for (uint64_t i = 0; i < Count; ++i) {
    int64_t DstOffBytes = static_cast<int64_t>(DstVO.ArrayBaseOffset) +
                          (DstElemOff + static_cast<int64_t>(i)) * Scale;
    DstFS[DstOffBytes] = FieldValue::scalar(PerElement);
    DstFS[DstOffBytes].setDeclaredType(ElemTy);
  }
...
// Sweep sibling VOs' FieldStates after a materialisation. Mirrors
// Graal's updateStatesForMaterialized (PartialEscapeClosure.java:654-670),
// which runs for EVERY materialisation — outer OR recursive — not just the
// recursive prerequisite path that the previous Jeandle code covered.
...
void Analyzer::materializeAt(jeandle::ObjectID ID,
  ...
  // out of per-pred VOs whose allocations live in different blocks. Graal
  // resolves this by ensureMaterialized()ing each per-pred VO at its end-of-
  // block and threading the per-pred materialized pointers through the
  // existing PHI; the PHI then BECOMES the materialized pointer for the
  // synthetic VO. Jeandle takes the simpler conservative path:
  ...
  //
  // Narrow cascade rule, mirroring Graal's
  //   PartialEscapeBlockState.materializeWithCommit (lines 323-333):
  //     int lockDepth = obj.getMaximumLockDepth();
  //     for (VirtualObjectNode other : ...)
  //       if (otherState.hasLocks() &&
  //           otherState.getMinimumLockDepth() < lockDepth)
  //         materializeWithCommit(other, ...);
  //
  // We use the per-LockEnter Order tag as the proxy for Graal's
  // MonitorIdNode.getLockDepth(): the OUTERMOST live lock on a VO is at
  ...
  if (HasLiveLocks && StrictLockOrder) {
    ...
    // Cascade by BytecodeDepth (Graal parity). When
    // !jeandle.lock_depth metadata is present, this is the true bytecode
    // monitor depth; otherwise BytecodeDepth carries the Order proxy value
    // at push time, so behavior is bit-identical to the earlier
    // implementation. The plan explicitly calls this comparison out as the
    // depth-aware narrow rule (matching Graal's getMinimumLockDepth /
    // getMaximumLockDepth comparison in PartialEscapeBlockState.
    // materializeWithCommit lines 323-333).
  ...
  // the field. Matches Graal's updateStatesForMaterialized which runs for
  // EVERY materialisation, not just the recursive prerequisite path.
...
void Analyzer::commit() {
  ...
  //                                           the escape path)
  // Mirrors Graal's EscapeClassification: NEVER vs PARTIAL vs ALWAYS map.
  // -------------------------------------------------------------------------
...
void Analyzer::processLoopExit(Loop *L) {
  // Mirrors Graal PartialEscapeClosure.processLoopExit (lines 737-766):
...
void Analyzer::processStateBeforeLoopOnOverflow(Loop *L) {
  // Mirrors Graal PartialEscapeClosure.processStateBeforeLoopOnOverflow
  // (PartialEscapeClosure.java:229-237): every VO still virtual at the
  // loop's forward end (preheader exit) is forcibly materialised, so the
  // re-do of the loop body in MATERIALIZE_ALL mode starts from a clean
  // "no live virtuals on entry" state.
...
void Analyzer::materializeAtPredFromExitInfo(
  ...
  // allocations and stores survive in IR. Graal's cascade-materialize path
  // (ensureMaterialized at every per-pred source + reuse the existing PHI as
  // the materialized pointer; PartialEscapeClosure.MergeProcessor.merge
  // lines 985-1001) is deferred.
  if (VObj.IsSynthetic) {
    Eligible[ID] = false;
    for (jeandle::ObjectID PID : VObj.SyntheticSourceIDs)
      Eligible[PID] = false;
  ...
  // Mirrors materializeAt's cascade above; see comment there for the
  // Graal-side justification (PartialEscapeBlockState.materializeWithCommit
  // lines 323-333).
  ...
  if (HasLiveLocks && StrictLockOrder) {
    ...
    assert(ThisIt != ExitInfo.LiveLockEnters.end() &&
           !ThisIt->second.empty() &&
           "LockCount > 0 implies a non-empty LiveLockEnters stack");
    // Cascade by BytecodeDepth (Graal parity). See the matching
    // comment in materializeAt above for the rationale.
    uint32_t ThisMaxDepth = ThisIt->second.back().BytecodeDepth;
    SmallVector<jeandle::ObjectID, 4> Cascade;
...
// ===========================================================================
//
// Mirrors Graal's EffectsClosure.processLoop (EffectsClosure.java:364-566) +
// PartialEscapeClosure.processStateBeforeLoopOnOverflow:
//
...
//     materialize every preheader-live virtual (same effect Graal achieves
//     by setting `EffectsClosureMode = MATERIALIZE_ALL` and re-walking),
//     then process the body once in MATERIALIZE_ALL mode so any new
//     allocations inside the body are also refused.
...
bool Analyzer::loopBlockExitsEquivalent(
  ...
  for (BasicBlock *BB : LoopBlocks) {
    ...
    if (HasA != HasB) {
      ...
      // verdict, mirroring Graal's EffectsClosure tolerance for a similar
      // edge case (knownAliveLoopEnds invariant, EffectsClosure.java).
...
void Analyzer::processLoop(Loop *L) {
  ...
  if (!Preheader) {
    ...
    //
    // The sound pessimistic action (and what Graal does for irreducible
    // regions: PartialEscapeAnalysis bails the entire EffectsClosure for
    // the affected region) is to MARK every VO still virtual at any
    ...
    // semantics (Graal's PartialEscapeAnalysis.canAnalyze returns false
    // for irreducible regions; the closure is skipped entirely there).
  ...
  // At TOP-LEVEL processLoop entry only (Graal's
  // EffectsClosure.processLoop loop.getDepth() == 1 gate), compute the
  // maximum loop depth within this nest. If it exceeds
  ...
  // at the bottom of this function (mirrors Graal lines 482-488 which
  // restore at depth==1 on convergence).
  ...
  if (Converged) {
    ...
    // virtuals.
    processLoopExit(L);
    // Restore mode on convergence (mirrors Graal EffectsClosure.java
    // 482-488 — currentMode reset to REGULAR at depth==1 after success).
    if (L->getLoopDepth() == 1)
  ...
  // the second observation, hard-bail (unreachable in debug). Mirrors
  // Graal EffectsClosure.java:553-564.
