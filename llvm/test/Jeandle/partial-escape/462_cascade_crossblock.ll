...
; EliminateStore; arr[i]=innerI (SYMBOLIC index) bails and marks arr+innerI
; ineligible; the arr escape is in `tail`. The pre-fix commit cascade read
; the per-block live FieldStates — which held only `tail`'s (empty) state —
; missed the arr->inner0 edge, left inner0 NeverEscapes, and the surviving
; real store wrote POISON into the escaped array. The persistent
; VirtualRefEdges set (recorded at processStore) now carries the edge across
; blocks: inner0 (and innerI) are kept real and the stores write the real
; pointers.
...
; Everything kept real: all three allocations retained; both stores write
...
; CHECK: store atomic ptr addrspace(1) %inner0, ptr addrspace(1) %slot0 unordered, align 4
