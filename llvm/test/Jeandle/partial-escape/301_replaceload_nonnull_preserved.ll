...
; PEA Round 5 (C9): when PEA virtualizes a reference field by RAUW-ing the
; field-load onto the previously-stored value, transfer load-only metadata
; (`!nonnull` etc.) from the original load onto the replacement load if the
; replacement is itself a LoadInst that lacks it. This preserves the precise
; "not-null" knowledge for subsequent LLVM passes after PEA snaps away the
; field-load.
;
; Setup:
...
; eliminated and its uses are RAUW'd onto %src). After C9 metadata
...
; %src and its !nonnull-carrying replacement survive. The C9 metadata copy
