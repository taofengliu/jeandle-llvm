...
; PEA-Plan §A4 + C8: lock-mismatch on A triggers the per-pred materialize;
; the then-pred's materialize for A then cascades through C8's narrow
...
; either A or B, so the A4 materialize at else for A doesn't cascade B.
;
; After this round of merge:
...
; The then-pred materializes BOTH B (cascade) and A (A4 direct), then
...
; live locks at else — the C8 rule needs HasLiveLocks). B at else stays
