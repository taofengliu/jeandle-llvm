...
; The transform's marker and outer poll are certificates, not proof. A signed
; decreasing inclusive loop needs the strict limit > INT_MIN margin because it
; executes the limit value before the final decrement.
...
; CHECK: SafepointCoverageVerifier: loop with header 'loop' in function 'forged_decreasing_non_strict_margin' has no dominating safepoint poll and no provable trip bound
; ABORT: SafepointCoverageVerifier: loop with header 'loop' in function 'forged_decreasing_non_strict_margin' has no dominating safepoint poll and no provable trip bound
