...
; RUN: opt -passes='safepoint-elimination<inclusive-loop-versioning>,safepoint-elimination<strip-mining>' \
; RUN:   -jeandle-enable-strip-mining -jeandle-enable-inclusive-loop-versioning=false \
; RUN:   -jeandle-safepoint-chunk-iters=8 -S < %s | FileCheck %s --check-prefix=DISABLED
...
define void @static_no_wrap_step2(i32 %start, i32 %limit) {
entry:
...
define void @marked_static_no_wrap_step2(i32 %start, i32 %limit) {
entry:
  %safe.limit = icmp slt i32 %limit, 2147483646
  br i1 %safe.limit, label %nonempty.check, label %exit
...
define void @runtime_inclusive_dynamic_step(i32 %limit, i32 %step) {
entry:
  br label %loop
loop:
...
define void @runtime_inclusive_zero_step(i32 %limit) {
entry:
  br label %loop
loop:
  %iv = phi i32 [ 0, %entry ], [ %iv.next, %loop ]
...
define void @runtime_inclusive_direction_mismatch(i32 %limit) {
entry:
  br label %loop
loop:
  %iv = phi i32 [ 0, %entry ], [ %iv.next, %loop ]
...
define void @runtime_inclusive_int_min_step(i32 %start, i32 %limit) {
entry:
  br label %loop
loop:
  %iv = phi i32 [ %start, %entry ], [ %iv.next, %loop ]
...
define void @runtime_inclusive_chunk_overflow(i32 %limit) {
entry:
  br label %loop
loop:
  %iv = phi i32 [ 0, %entry ], [ %iv.next, %loop ]
...
define void @runtime_inclusive_unsigned_step2(i32 %limit) {
entry:
  br label %loop
loop:
  %iv = phi i32 [ 0, %entry ], [ %iv.next, %loop ]
...
define void @runtime_inclusive_i64_step2(i64 %limit) {
entry:
  br label %loop
loop:
...
define void @runtime_ne_step2(i32 %limit) {
entry:
  br label %loop
loop:
  %iv = phi i32 [ 0, %entry ], [ %iv.next, %loop ]
