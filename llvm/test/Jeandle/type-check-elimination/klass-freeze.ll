; RUN: opt -S -passes="type-check-elimination" -jeandle-vm-callback-log=%S/Inputs/klass-freeze.cblog %s 2>&1 | FileCheck %s

; Test: SuperKlass wrapped in a freeze instruction. extractKlassConstant should
; look through freeze to find the underlying inttoptr constant.
...
entry:
  ...
  %raw_ptr = inttoptr i64 4 to ptr
  %klass_ptr = freeze ptr %raw_ptr
